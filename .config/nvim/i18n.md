...
yelog
yelog
fix(parser): normalize Windows autocmd patterns
117ba03
 · 
3 days ago
doc
docs: document named export locale modules
last week
lua/i18n
fix(parser): normalize Windows autocmd patterns
3 days ago
.gitignore
chore: ignore codegraph cache
2 weeks ago
.nvimlog
feat: add usage.notify_no_key option to silence missing-key warning
8 months ago
AGENTS.md
update agents
5 months ago
CLAUDE.md
feat: init claude agent
5 months ago
LICENSE
chore: add Apache-2.0 LICENSE and update README
9 months ago
README.md
docs: document named export locale modules
last week
...
Show Translation	With blink.cmp
	
Diagnostic	Help like vim.lsp.buf.signature_help()
	
Jump like vim.lsp.buf.definition()	With fzf-lua
	

Add Missing Key	Show Other Translations
...
🚀 Quickstart
Configure locales + sources (or enable auto_detect).
Open a file and run :I18nNextLocale / :I18nShowTranslations.
Use the picker I18n.i18n_keys() or jump to definitions/usages.
🧭 Commands
Command	Description
...
Place the cursor on an i18n call whose key does NOT exist (e.g. t('system.new_feature.title')).
Run :I18nAddKey.
Fill one input line per locale (first = default).
<Tab> / <S-Tab> moves between locales; <Enter> writes; <Esc> cancels.
Notes:
...
Tables unlock additional control: { call = 'i18n.t', quotes = { "'", '"' }, allow_whitespace = false }.
...
Submit PRs with unit-tested or manually verified changes.
Keep coding style consistent with the repository.
📄 License
...
Topics
i18n plugin internationalization neovim neovim-plugin
Resources
 Readme
License
 Apache-2.0 license
